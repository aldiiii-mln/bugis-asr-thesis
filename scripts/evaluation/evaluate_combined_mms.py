"""
evaluate_combined_mms.py

Sama persis polanya dengan evaluate_combined_whisper.py / evaluate_authentic_only_mms.py
-- reuse compute_wer_cer.py + bootstrap_ci.py lewat import, cuma titik masuk
(path CSV prediksi) & baseline Zero-Shot yang beda. Logikanya tidak berubah
sama sekali antar kondisi/model.
...
# ---- EDIT BAGIAN INI ----
PREDICTIONS_CSV = "/content/drive/MyDrive/bugis_authentic/combined_results_mms/combined_mms_predictions.csv"
RECORDING_MAPPING_CSV = "/content/drive/MyDrive/bugis_authentic/final_recording_mapping.csv"
OUTPUT_JSON = "/content/drive/MyDrive/bugis_authentic/combined_results_mms/combined_mms_bootstrap_ci.json"
GROUPING_DIAGNOSTIC_CSV = "/content/drive/MyDrive/bugis_authentic/combined_results_mms/grouping_diagnostic_combined_mms.csv"

N_BOOTSTRAP = 1000
CI_LEVEL = 0.95

# Baseline Zero-Shot MMS -- ANGKA SAMA PERSIS dgn evaluate_authentic_only_mms.py
# dan evaluate_synthetic_only_mms.py.
...
def main():
    ...
    n = len(file_ids)
    print(f"Combined MMS: {n} baris prediksi dimuat dari {PREDICTIONS_CSV}")
    if n != 106:
    ...
    print(f"Rekaman unik terdeteksi: {n_groups} (ekspektasi 40, atau kurang kalau ada fold yang skip)")
