"""
evaluate_synthetic_only_mms.py

Sama persis polanya dengan evaluate_synthetic_only_whisper.py -- reuse
compute_wer_cer.py + bootstrap_ci.py lewat import, cuma titik masuk (path
CSV prediksi) & baseline Zero-Shot yang beda. Baca docstring
evaluate_synthetic_only_whisper.py / evaluate_authentic_only_mms.py kalau
butuh penjelasan lebih lengkap soal kenapa driver terpisah ini perlu, dan
kenapa logika bootstrap_ci-nya tidak berubah sama sekali antar kondisi.
...
# ---- EDIT BAGIAN INI ----
PREDICTIONS_CSV = "/content/drive/MyDrive/bugis_authentic/synthetic_only_results/synthetic_only_mms_predictions.csv"
RECORDING_MAPPING_CSV = "/content/drive/MyDrive/bugis_authentic/final_recording_mapping.csv"   # SAMA persis, test set tidak berubah
OUTPUT_JSON = "/content/drive/MyDrive/bugis_authentic/synthetic_only_results/synthetic_only_mms_bootstrap_ci.json"
GROUPING_DIAGNOSTIC_CSV = "/content/drive/MyDrive/bugis_authentic/synthetic_only_results/grouping_diagnostic_synthetic_only_mms.csv"

N_BOOTSTRAP = 1000
CI_LEVEL = 0.95

# Baseline Zero-Shot MMS -- ANGKA SAMA PERSIS dgn evaluate_authentic_only_mms.py
# (rangkuman metodologi 9.8, N=106, bootstrap 95% CI group-aware, n_bootstrap=1000).
...
def main():
    ...
    n = len(file_ids)
    print(f"Synthetic-Only MMS: {n} baris prediksi dimuat dari {PREDICTIONS_CSV}")
    if n != 106:
